...
def compute_category_breakdown(sim_matrix, targets, metadata):
    cat_col = next((c for c in ["domain", "coordinate", "superordinate"] if c in metadata.columns), None)
    if not cat_col:
        return {}
...
def compute_hierarchical_breakdown(sim_matrix, targets, metadata):
    ...
    coordinate_col = next((c for c in ["coordinate", "coordinate"] if c in meta_aligned.columns), "coordinate")
    ...
    concept_to_coordinate = meta_aligned.drop_duplicates(spec_col).set_index(spec_col)[coordinate_col].to_dict()
    ...
    true_coordinates = [concept_to_coordinate.get(c) for c in true_concepts]
    true_domains = [concept_to_domain.get(c) for c in true_concepts]

    pred_coordinates = [concept_to_coordinate.get(c) for c in pred_concepts]
    ...
    coord_err_count = int(sum((tc != pc) and (tb == pb) for tc, pc, tb, pb in zip(true_concepts, pred_concepts, true_coordinates, pred_coordinates)))
    super_err_count = int(sum((tb != pb) and (td == pd) for tb, pb, td, pd in zip(true_coordinates, pred_coordinates, true_domains, pred_domains)))
    ...
    return {
        ...
        "top1_coordinate_acc": float(sum(tb == pb for tb, pb in zip(true_coordinates, pred_coordinates)) / total) if total > 0 else 0.0,
...
def compute_top10_breakdown_depth(sim_matrix, targets, metadata, unique_concepts):
    ...
    same_coordinate_counts, same_domain_counts = [], []
    ...
    for i, top_k in enumerate(top10_indices):
        ...
        same_coordinate_counts.append(sum(1 for b in retrieved_coordinates if b == target_coordinate) / float(top10_k))
